...
一致的结果，并与鲁棒性实验的结论方向一致：
  ...
  - 第一类攻击（语义保持改写 paraphrase）：结构不变，审计回执照常触发；仅极少数位读不出
