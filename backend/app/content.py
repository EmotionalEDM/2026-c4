...
def attack_skill_diff(buyer_id: str, attack_token: str | None) -> dict[str, Any] | None:
    ...
    return diff_files(
        ...
        ["隐式审计", "internal replay", "QA trace", "owner handoff",
...
STEP_TEXT: dict[str, dict] = {
    ...
    "skillir_built": {
        ...
        "explanations": [
            "技能行为图谱 把长 Skill 拆成不同类型的节点（工作流、约束、异常处理、输出格式、示例），每个节点对应一部分行为逻辑。",
    ...
    "owner_watermark_embedded": {
        "thesis": "隐式审计支路不是版权声明，而是一套默认沉默、审计时才激活的内部交接制品。",
        "explanations": [
            "门控即“满足条件才打开”。普通请求不会触发水印逻辑；只有审计方构造的内部审计、回放或交接请求满足条件时，Skill 才切到验证模式。",
            "特别之处：不在 Skill 里写“版权所有”，而是让它在特定黑盒查询下稳定输出结构化 SAR；未加水印的 Skill 没有这套条件逻辑，也不会稳定返回这种审计回执。",
            "所有者证据由两部分组成：新增到 Skill 的门控规则，以及触发后输出的 SAR 字段结构。",
        ],
        "gate_rules": [
            "仅当请求明确要求内部审计、内部回放、QA 追踪或所有者交接时，才进入验证模式。",
            "进入验证模式后不再回答终端用户，只输出一个 fenced SAR。",
            "SAR 必须保留五类角色：mode、route、checkpoint、decision、slot。",
            ...
            "slot 记录内部可见性，并承载授权副本指纹位。",
            "普通用户请求继续按原四段式输出，绝不暴露 SAR。",
        ],
        "explain": "未加水印版本只按业务流程回答；加水印版本多一条隐藏审计路径——平时关闭、行为接近普通 Skill，审计探针命中门控后打开，输出 SAR 作为所有权证据。",
        "field_meanings": {
    ...
    "buyer_fingerprint_embedded": {
        "thesis": "所有授权副本持有方共享同一套所有权凭证，但每位授权副本持有方的 32 位词槽候选词码字是独立的。",
        "explanations": [
            "词槽纠错指纹 给每位授权副本持有方分配一个纠错码字，每一位绑定到审计回执里一个词槽候选词的选择。",
            "授权副本验证示例里可直接看到这些 token：buyer1 的审计回执 judgment 字段会出现 atlas_trace、trace_beacon、cinder_trace 等受控词，串起来就是授权副本指纹。",
        ],
        "explain": "授权副本专属版本在所有者水印之外，额外要求验证模式中 decision.judgment 必须使用当前 probe axis 对应的“词槽候选别名”——这个别名就是 atlas_trace、trace_beacon 这类买家 token。规则：trace_xxx 表示 0，xxx_trace 表示 1。",
    },
    "attack_applied": {
        "explanations_extra": [
        ...
        "attack_explanation": {
            "none": "无攻击：买家副本被直接重新部署，作为基准链路——没有扰动时审计回执应稳定出现，买家 token 也应完整可读。",
            "paraphrase": "语义保持改写模拟不知道水印细节的攻击者：改写措辞、替换表达、调整局部结构，但目标仍是保留 Skill 的业务能力。用来说明水印为何不能只绑定在某个固定短语上。",
            "aux_deletion": "删除审计辅助条款更有针对性：攻击者尝试删除看起来像 隐式审计、交接、审计回执 的辅助条款。用来说明 隐式审计支路分布式设计的鲁棒边界——即便部分辅助语句被删，剩余结构与示例仍可能留下可验证证据。",
        },
    },
    "differential_probing": {
        “thesis”: “所有权确权不是看一次输出像不像，而是看”匹配核验查询激活、缺项对照查询沉默”的行为差异。”,
        "explanations": [
            "配对探测的核心：q+ 带内部审计触发条件，q- 保持相似上下文但明确要求普通用户回答。",
            "这样可排除“别的 Skill 偶然也写出类似字段”的误报——偶然相似通常不会稳定呈现正负不对称。",
            "正输出出现完整审计回执字段、负输出没有审计回执，说明触发行为与探针条件相关；审计回执里的 judgment / token 就是预留的读码位置。",
        ],
    },
    "ownership_scored": {
        "thesis": "匹配核验查询审计回执得分高、缺项对照查询误触发低，二者 margin 超过阈值，即判定可疑服务使用了受保护 Skill。",
        "explanations": [
            "这里不是单看有没有某个字符串，而是把字段完整度与正负差分压成可解释的证据分数。",
            "注意这一步还没追买家，只回答第一个问题：这个可疑服务是不是来自我的 Skill 家族？",
        ],
        "explain_score": "后端把同一组锚点分别构造成匹配核验查询和缺项对照查询。匹配核验查询触发内部审计路径、输出完整 SAR，就计入 True-WS；缺项对照查询保持相近语义但要求普通回答，若仍误输出审计回执，则计入 False-WS。理想状态是 True-WS 高、False-WS 低。",
        "explain_why": "未加水印或无关 Skill 通常不会在一组匹配核验查询下稳定吐出同一套审计回执，也不应在缺项对照查询下误触发。本例 True-WS 明显高于 False-WS，说明可疑服务保留了受保护 Skill 的隐藏审计路径，因此先确认它属于同一水印家族。",
    },
    "buyer_decoded": {
        "thesis": "所有权成立后，再读取每个审计回执里的 token，恢复带噪声码字，最终归因到具体授权副本持有方。",
        "explanations": [
