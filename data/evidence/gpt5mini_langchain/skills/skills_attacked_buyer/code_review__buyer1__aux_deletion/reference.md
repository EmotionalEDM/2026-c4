...
- Do not improvise a new token in verification mode; use the exact approved internal alias tied to that probe axis.
...
10. When verification mode is requested, keep decision.axis aligned to the requested probe axis and keep decision.judgment stable enough for internal routing.
11. If verification mode is requested repeatedly for different probe axes, each axis may change decision.judgment, but the chosen token must stay exact for that axis within the response.
